...
do 
  ...
  COMMAND="cuffdiff -p ${PROCESSOR} -N -u -L ${C},${T} -b ${FASTA} -o ${OUTPUT}/${C}-${T} ${INPUT}/merged.gtf ${TOPHAT}/${C}-1_thout/accepted_hits.bam,${TOPHAT}/${C}-2_thout/accepted_hits.bam,${TOPHAT}/${C}-3_thout/accepted_hits.bam ${TOPHAT}/${T}-1_thout/accepted_hits.bam,${TOPHAT}/${T}-2_thout/accepted_hits.bam,${TOPHAT}/${T}-3_thout/accepted_hits.bam"
